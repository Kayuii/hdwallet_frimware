...
static uint32_t m_word_count;
static bool m_awaiting_word = false;
...
void next_word(void) {
    ...
    snprintf(title_formatted, SMALL_STR_BUF, "Device Recovery Step %lu/24",
             (unsigned long)(word_index + 1));
    ...
    if (word_pos == 0) {
        ...
        snprintf(body_formatted, MEDIUM_STR_BUF, "Enter the word \"%s\".",
                 fake_word);

        layout_standard_notification(title_formatted, body_formatted,
        ...
        snprintf(body_formatted, MEDIUM_STR_BUF,
                 "Enter the %lu%s of your recovery sentence.",
                 (unsigned long)word_pos, desc);
...
void recovery_init(uint32_t _word_count, bool passphrase_protection,
    ...
    if (_word_count != 12 && _word_count != 18 && _word_count != 24) {
        fsm_sendFailure(FailureType_Failure_SyntaxError,
                        "Invalid word count (has to be 12, 18 or 24");
        layout_home();
        return;
    }

    m_word_count = _word_count;
    ...
    for (i = m_word_count; i < 24; i++) {
        word_order[i] = 0;
    }
    random_permute_char(word_order, 24);
    m_awaiting_word = true;
    word_index = 0;
    next_word();
}
...
void recovery_word(const char *word) {
    ...
    volatile bool isCorrectFake =
        exact_str_match(word, fake_word, strlen(word) + 1);
    ...
    if (word_pos == 0) {
        ...
        if (enforce_wordlist & (!found)) {
            storage_reset();
            fsm_sendFailure(FailureType_Failure_SyntaxError,
                            "Word not found in the bip39 wordlist");
            layout_home();
            return;
        }
    ...
    if (word_index + 1 == 24) {
        // last one
        storage_setMnemonicFromWords(words, m_word_count);

        const char *entered_mnemonic = storage_getShadowMnemonic();
        if (entered_mnemonic &&
            (!enforce_wordlist || mnemonic_check(entered_mnemonic))) {
            storage_commit();
...
void recovery_abort(bool send_failure) {
    if (m_awaiting_word || recovery_cipher_abort()) {
        m_awaiting_word = false;
