...
void send_fsm_co_error_message(int co_error) {
    switch (co_error) {
        ...
        case (TXOUT_EXCHANGE_CONTRACT_ERROR): {
            switch (get_exchange_error()) {
                ...
                case ERROR_EXCHANGE_CANCEL: {
                    fsm_sendFailure(FailureType_Failure_ActionCancelled,
                                    "Exchange transaction cancelled");
                    break;
                }
                case ERROR_EXCHANGE_RESPONSE_STRUCTURE: {
                    fsm_sendFailure(FailureType_Failure_Other,
                                    "Obsolete Response structure error");
                    break;
                }
                default:
        ...
        default: {
            fsm_sendFailure(FailureType_Failure_Other,
                            "Unknown TxOut compilation error");
            break;
        }
    }
}
...
bool compile_input_script_sig(TxInputType *tinput) {
    ...
    if (tinput->has_multisig) {
        tinput->script_sig.size = compile_script_multisig(
            coin, &(tinput->multisig), tinput->script_sig.bytes);
    } else {  // SPENDADDRESS
        uint8_t hash[20];
        ecdsa_get_pubkeyhash(node.public_key, curve->hasher_pubkey, hash);
        tinput->script_sig.size = compile_script_sig(coin->address_type, hash,
...
void signing_init(const SignTx *msg, const CoinType *_coin,
    ...
    animating_progress_handler();  // layoutProgressSwipe(_("Signing
                                   // transaction"), 0);
...
static bool signing_check_output(TxOutputType *txoutput) {
    ...
    if (txoutput->address_n_count > 0) {
         ...
         * paid. this was added during the times segwit was not yet fully
         * activated to make sure the user is not tricked to use witness change
         * output instead of regular one therefore creating ANYONECANSPEND
         * output
         */
    ...
    int co = run_policy_compile_output(coin, root, txoutput, &bin_output,
                                       !is_change);
    if (!is_change) {
        animating_progress_handler();  // layoutProgress(_("Signing
...
static bool signing_sign_hash(TxInputType *txinput, const uint8_t *private_key,
    ...
    resp.serialized.has_serialized_tx = true;
    if (ecdsa_sign_digest(curve->params, private_key, hash, sig, NULL, NULL) !=
        0) {
...
static bool signing_sign_segwit_input(TxInputType *txinput) {
    ...
    if (txinput->script_type == InputScriptType_SPENDWITNESS ||
        ...
        resp.has_serialized = true;
        if (!signing_sign_hash(txinput, node.private_key, node.public_key,
                               hash))
            return false;
        ...
        if (txinput->has_multisig) {
            ...
            r +=
                ser_length(script_len, resp.serialized.serialized_tx.bytes + r);
            r += compile_script_multisig(
                coin, &txinput->multisig,
                resp.serialized.serialized_tx.bytes + r);
...
void signing_txack(TransactionType *tx) {
    ...
    switch (signing_stage) {
        case STAGE_REQUEST_1_INPUT:
            ...
            if (tx->inputs[0].script_type == InputScriptType_SPENDMULTISIG ||
                ...
                if (idx1 > 0 && to.is_segwit == true) {
                    fsm_sendFailure(FailureType_Failure_SyntaxError,
                                    _("Mixing segwit and non-segwit inputs is "
                                      "not allowed"));
                    signing_abort();
                    return;
                }
                ...
                if (coin->force_bip143 || overwintered) {
                    ...
                    // remember the first non-segwit input -- this is the first
                    // input we need to sign during phase2
        ...
        case STAGE_REQUEST_2_PREV_META:
            ...
            tx_init(&tp, tx->inputs_cnt, tx->outputs_cnt, tx->version,
                    tx->lock_time, tx->expiry, tx->extra_data_len,
                    curve->hasher_sign, overwintered, version_group_id);
            if (coin->decred) {
        ...
        case STAGE_REQUEST_2_PREV_OUTPUT:
            ...
            if (idx2 == input.prev_index) {
                ...
                if (coin->decred &&
                    tx->bin_outputs[0].decred_script_version > 0) {
                    fsm_sendFailure(FailureType_Failure_SyntaxError,
                                    _("Decred script version does not match "
                                      "previous output"));
                    signing_abort();
                    return;
                }
        ...
        case STAGE_REQUEST_4_INPUT:
            progress =
                500 +
                ((signatures * progress_step + idx2 * progress_meta_step) >>
                 PROGRESS_PRECISION);
            if (idx2 == 0) {
                tx_init(&ti, inputs_count, outputs_count, version, lock_time,
                        expiry, 0, curve->hasher_sign, overwintered,
                        version_group_id);
                hasher_Reset(&hasher_check);
            }
            // check prevouts and script type
            tx_prevout_hash(&hasher_check, tx->inputs);
            hasher_Update(&hasher_check,
                          (const uint8_t *)&tx->inputs[0].script_type,
        ...
        case STAGE_REQUEST_4_OUTPUT:
            progress = 500 + ((signatures * progress_step +
                              ...
                              PROGRESS_PRECISION);
            int co = run_policy_compile_output(coin, root, tx->outputs,
                                               &bin_output, false);
            if (co <= TXOUT_COMPILE_ERROR) {
                send_fsm_co_error_message(co);
                signing_abort();
            ...
            if (idx2 < outputs_count - 1) {
                ...
                signatures++;
                progress =
                    500 + ((signatures * progress_step) >> PROGRESS_PRECISION);
                animating_progress_handler();  // layoutProgress(_("Signing
                                               // transaction"), progress);
                update_ctr = 0;
                if (idx1 < inputs_count - 1) {
            ...
            if (tx->inputs[0].script_type == InputScriptType_SPENDMULTISIG ||
                ...
                if (overwintered) {
                    switch (version) {
                        ...
                        default:
                            fsm_sendFailure(FailureType_Failure_SyntaxError,
                                            _("Unsupported version for "
                                              "overwintered transaction"));
                            signing_abort();
                            return;
                    }
                } else {
                ...
                // fixup normal p2pkh script into witness 0 p2wpkh script for
                // p2sh we convert 76 A9 14 <digest> 88 AC  to 16 00 14 <digest>
                ...
                tx->inputs[0].script_sig.bytes[2] =
                    0x20;  // push 32 bytes (digest)
                // compute digest of multisig script
                if (!compile_script_multisig_hash(
                        coin, &tx->inputs[0].multisig,
        ...
        case STAGE_REQUEST_DECRED_WITNESS:
            ...
            if (idx1 == 0) {
                // witness
                tx_init(&to, inputs_count, outputs_count, version, lock_time,
                        expiry, 0, curve->hasher_sign, overwintered,
                        version_group_id);
                to.is_decred = true;
            }
