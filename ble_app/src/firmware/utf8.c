...
static int trailing_bytes_for_utf8(const unsigned char c)
{
    if (c >= 0xFC) {
        return 5;
    }
...
{
    return trailing_bytes_for_utf8((const unsigned char)s[0]) + 1;
}

/* conversions without error checking
   only works for valid UTF-8, i.e. no 5- or 6-byte sequences
   srcsz = source size in bytes, or -1 if 0-terminated
   sz = dest size in # of wide characters need srcsz+1
...
int u8_toucs(uint32_t *dest, int sz, char *src, int srcsz)
{
    ...
    while (i < sz - 1) {
        nb = trailing_bytes_for_utf8((const unsigned char)*src);
        if (srcsz == -1) {
            if (*src == 0)
                goto done_toucs;
        }
