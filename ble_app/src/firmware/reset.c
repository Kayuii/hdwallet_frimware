...
void reset_init(bool display_random, uint32_t _strength,
    ...
    if (_strength != 128 && _strength != 192 && _strength != 256) {
        NRF_LOG_DEBUG("fsm_sendFailure reset init");
    ...
    if (no_backup) {
        ...
        if (!confirm(ButtonRequestType_ButtonRequest_Other, "WARNING",
            ...
            NRF_LOG_DEBUG("fsm_sendFailure no_backup");
    ...
    if (display_random) {
        if (!confirm(ButtonRequestType_ButtonRequest_ResetDevice,
                     "Internal Entropy", "%s %s %s %s", ent_str[0], ent_str[1],
                     ent_str[2], ent_str[3])) {
            fsm_sendFailure(FailureType_Failure_ActionCancelled,
                            "Reset cancelled");
            layout_home();
...
void reset_entropy(const uint8_t *ext_entropy, uint32_t len) {
    ...
    while (tok) {
        ...
        if (calc_str_line(font, mnemonic_display, BODY_WIDTH) > 4) {
            page_count++;
        ...
        if (strlen(mnemonic_by_screen[page_count]) == 0) {
            strlcpy(mnemonic_by_screen[page_count], tok,
                    MNEMONIC_BY_SCREEN_BUF);
        } else {
            strlcat(mnemonic_by_screen[page_count], " ",
                    MNEMONIC_BY_SCREEN_BUF);
            strlcat(mnemonic_by_screen[page_count], tok,
                    MNEMONIC_BY_SCREEN_BUF);
        }
    ...
    for (uint32_t current_page = 0; current_page < page_count; current_page++) {
        ...
        if (page_count > 1) {
            ...
            snprintf(title, MEDIUM_STR_BUF,
                     "Recovery Sentence %" PRIu32 "/%" PRIu32 "",
