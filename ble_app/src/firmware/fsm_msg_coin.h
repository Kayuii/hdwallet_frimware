void fsm_msgGetPublicKey(GetPublicKey *msg) {
    ...
    HDNode *node = fsm_getDerivedNode(curve, msg->address_n,
    ...
    if (coin->xpub_magic && script_type == InputScriptType_SPENDADDRESS) {
        ...
        hdnode_serialize_public(node, fingerprint,
                                coin->xpub_magic_segwit_native, resp->xpub,
                                sizeof(resp->xpub));
    } else {
    ...
    if (msg->has_show_display && msg->show_display) {
        ...
        if (!confirm_xpub(node_str, resp->xpub)) {
            fsm_sendFailure(FailureType_Failure_ActionCancelled,
                            "Show extended public key cancelled");
            layout_home();
            return;
        }
    }
    ...
    if (node) memzero(node, sizeof(node));
}
...
void fsm_msgGetAddress(GetAddress *msg) {
    ...
    if (msg->has_multisig) {           // use progress bar only for multisig
        animating_progress_handler();  // layoutProgress(_("Computing address"),
                                       // 0);
    }
    ...
    if (msg->has_show_display && msg->show_display) {
        ...
        if (msg->has_multisig) {
            snprintf(node_str, sizeof(node_str),
                     "Multisig (%" PRIu32 " of %" PRIu32 ")", msg->multisig.m,
                     (uint32_t)msg->multisig.pubkeys_count);
        } else {
            if (!bip32_node_to_string(node_str, sizeof(node_str), coin,
                                      msg->address_n, msg->address_n_count,
                                      /*whole_account=*/false) &&
        ...
        if (mismatch) {
            if (!confirm(ButtonRequestType_ButtonRequest_Other, "WARNING",
                         "Wrong address path for selected coin. Continue at "
                         "your own risk!")) {
        ...
        size_t prefix_len =
            coin->has_cashaddr_prefix ? strlen(coin->cashaddr_prefix) + 1 : 0;
