...
bool is_token_transaction(const EthereumSignTx *msg) {
    return msg->has_token_shortcut && msg->has_token_value &&
           (msg->has_token_to || msg->to_address_n_count > 0);
}
...
static void layoutEthereumConfirmTx(const uint8_t *to, uint32_t to_len,
    ...
    if (out_str_len <= (size_t)snprintf(out_str, out_str_len, "Send %s to %s",
                                        amount,
...
static void layoutEthereumData(const uint8_t *data, uint32_t len,
    ...
    if ((uint32_t)snprintf(out_str, out_str_len, "%s%s%s%s", hexdata[0],
                           hexdata[1], hexdata[2],
                           summarystart) >= out_str_len) {
        /*error detected.  Clear the buffer */
        memset(out_str, 0, out_str_len);
    }
}
...
void ethereum_signing_init(EthereumSignTx *msg, const HDNode *node,
    ...
    if ((msg->has_token_to && msg->token_to.size == 20) &&
        msg->value.size == 0 && data_total == 0 &&
        msg->data_initial_chunk.size == 0 && msg->has_token_to &&
        msg->has_token_shortcut) {
        if (!tokenByTicker(chain_id, msg->token_shortcut, &token)) {
            fsm_sendFailure(FailureType_Failure_SyntaxError,
                            _("Cannot uniquely determine token from ticker"));
            ethereum_signing_abort();
            return;
        }
        ...
        memcpy(
            msg->data_initial_chunk.bytes + 36 + (32 - msg->token_value.size),
            msg->token_value.bytes, msg->token_value.size);
    ...
    // detect ERC-20 token
    if (msg->to.size == 20 && msg->value.size == 0 && data_total == 68 &&
    ...
    if (token == NULL && data_total > 0) {
        ...
        if (!storage_isPolicyEnabled("AdvancedMode")) {
            (void)review(
                ...
                "Signing of arbitrary ETH contract data is recommended only "
                "for "
