static int process_ethereum_xfer(const CoinType *coin, EthereumSignTx *msg) {
    ...
    if (msg->has_to || msg->to.size || strlen((char *)msg->to.bytes) != 0 ||
        msg->has_token_to || msg->token_to.size ||
        strlen((char *)msg->token_to.bytes) != 0)
        return TXOUT_COMPILE_ERROR;

    char node_str[NODE_STRING_LENGTH];
    if (!bip32_node_to_string(node_str, sizeof(node_str), coin,
                              msg->to_address_n, msg->to_address_n_count,
                              /*whole_account=*/false))
        return TXOUT_COMPILE_ERROR;
    ...
    if (is_token_transaction(msg)) {
        ...
        to_size = (size_t *)&msg->token_to.size;
        to_bytes = msg->token_to.bytes;
        value_bytes = msg->token_value.bytes;
        value_size = (size_t *)&msg->token_value.size;
        ...
        has_to = &msg->has_to;
        to_size = (size_t *)&msg->to.size;
        to_bytes = msg->to.bytes;
        value_bytes = msg->value.bytes;
        value_size = (size_t *)&msg->value.size;
        token = NULL;
    }
    ...
    ethereumFormatAmount(&value, token, chain_id, amount_str,
                         sizeof(amount_str));
        ...
        switch (msg->address_type) {
            case OutputAddressType_EXCHANGE: {
                ...
                HDNode *root_node = fsm_getDerivedNode(SECP256K1_NAME, 0, 0,
                                                       NULL); /* root node */
                ret_result = run_policy_compile_output(
                    coin, root_node, (void *)msg, (void *)NULL, true);
...
void fsm_msgEthereumGetAddress(EthereumGetAddress *msg) {
    ...
    if (!hdnode_get_ethereum_pubkeyhash(node, resp->address.bytes)) return;
...
void fsm_msgEthereumVerifyMessage(const EthereumVerifyMessage *msg) {
    ...
    if (!confirm(ButtonRequestType_ButtonRequest_Other, "Message Verified",
                 "%s", msg->message.bytes)) {
        fsm_sendFailure(FailureType_Failure_ActionCancelled, NULL);
        layout_home();
        return;
